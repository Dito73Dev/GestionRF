	...
	*  For AD it is <provider code><username>
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_aldevindiv_sol_OwnerID", $userId );
						storageSet( "_Mi_inventario_OwnerID", $userId );
						storageSet( "_aldevindiv_sol_sign_a_OwnerID", $userId );
						storageSet( "_aldevindiv_sol_sign_b_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["Username"] );
			storageSet( "_aldevindiv_sol_OwnerID", $data["Username"] );
						storageSet( "_Mi_inventario_OwnerID", $data["usr_personalid"] );
						storageSet( "_aldevindiv_sol_sign_a_OwnerID", $data["Username"] );
						storageSet( "_aldevindiv_sol_sign_b_OwnerID", $data["Username"] );
		}
	}
